...
$markerGroups = @(
    [pscustomobject]@{ Id = 'S-01'; EvidenceClass = 'Static'; Files = @(
        @{ Path = 'src/HerdrOps.Infrastructure/Storage/HerdrStateStoreModels.cs'; Markers = @('CurrentSchemaVersion = 4') },
        @{ Path = 'src/HerdrOps.Infrastructure/Storage/SqliteHerdrStateStore.cs'; Markers = @('MigrationName =', 'AssignmentLifecycleMigrationName', 'ApplyMigrations', 'GetMigration', '1 => new MigrationDefinition', '2 => new MigrationDefinition', '3 => new MigrationDefinition', '4 => new MigrationDefinition', 'ComplianceReviewMigrationName', 'ValidateMigrationHistory', 'ComputeMigrationSha256', 'script_sha256', 'PRAGMA user_version', 'BeginTransaction', 'transaction.Commit()', 'transaction.Rollback()', 'version > HerdrStateStoreOptions.CurrentSchemaVersion') },
        ...
        @{ Path = 'src/HerdrOps.Infrastructure/Storage/SqliteHerdrStateStore.ComplianceReviewMigration.cs'; Markers = @('ComplianceReviewMigrationName', 'ComplianceReviewMigrationSql', 'CREATE TABLE compliance_review_incidents', 'CREATE TABLE compliance_review_events') },
        ...
        @{ Path = 'tests/HerdrOps.IntegrationTests/ComplianceReviewStorageTests.cs'; Markers = @('SchemaVersionFourCreatesGuardedRoleReviewLedger', 'GetMigrationForTesting(4)', 'Assert.AreEqual(4, store.GetDiagnostics().SchemaVersion)') }
    ) },
...
Test-SelectedTest -Id 'C-04' -Name 'schema' -EvidenceClass 'LocalSQLiteIntegration' -Project 'tests/HerdrOps.IntegrationTests/HerdrOps.IntegrationTests.csproj' -Filter 'FullyQualifiedName~SqliteHerdrStateStoreTests|FullyQualifiedName~ComplianceReviewStorageTests.SchemaVersionFourCreatesGuardedRoleReviewLedger'
