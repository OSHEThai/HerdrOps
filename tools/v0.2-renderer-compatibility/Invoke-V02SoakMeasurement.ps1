...
if(-not$Synthetic){
    ...
    $server=[Diagnostics.Process]::GetCurrentProcess();$serverPath=[IO.Path]::GetFullPath($server.MainModule.FileName);$serverSha=(Get-FileHash -LiteralPath $serverPath -Algorithm SHA256).Hash.ToUpperInvariant()
    ...
    $clientPid=Wait-RendererTargetObservationPipe $telemetryPipe 60;if([int]$clientPid-ne$appProcess.Id){throw 'Soak telemetry pipe was not connected by the launched packaged App PID.'}
    ...
    Assert-SoakExactProperties $hello @('schemaVersion','kind','runNonce','sourceCommit','sourceTree','packageIdentitySha256','packageArchiveSha256','serverProcessId','app','renderer') 'Soak producer hello'
