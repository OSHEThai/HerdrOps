    ...
    foreach ($language in @('Thai', 'English')) {
        ...
        foreach ($name in $script:RendererCaptureNames) {
            ...
            if ($captureMode -eq 'LiveOperator') {
                ...
                if ($firstSourceIdentity.Bytes -ne $sourceIdentity.Bytes -or $firstSourceIdentity.Sha256 -cne $sourceIdentity.Sha256 -or $firstSourceIdentity.Width -ne $sourceIdentity.Width -or $firstSourceIdentity.Height -ne $sourceIdentity.Height -or $firstSourceIdentity.FinalPath -cne $sourceIdentity.FinalPath -or $firstSourceIdentity.FileIdentity -cne $sourceIdentity.FileIdentity -or $firstSourceIdentity.LinkCount -ne 1 -or $sourceIdentity.LinkCount -ne 1 -or $firstSourceIdentity.LastWriteTimeUtc -ne $sourceIdentity.LastWriteTimeUtc) { throw "Target capture '$captureKey' changed between stable identity reads." }
                $sourceObservedUtc = ([DateTimeOffset]$sourceIdentity.LastWriteTimeUtc).ToUniversalTime().ToString('O', [Globalization.CultureInfo]::InvariantCulture)
                if ($sourceIdentity.Bytes -ne [long]$targetBinding.bytes -or $sourceIdentity.Sha256 -cne [string]$targetBinding.sha256 -or $sourceIdentity.Width -ne [int]$targetBinding.widthPixels -or $sourceIdentity.Height -ne [int]$targetBinding.heightPixels -or $sourceIdentity.FileIdentity -cne [string]$targetBinding.fileIdentity -or [long]$targetBinding.linkCount -ne 1 -or $sourceObservedUtc -cne [string]$targetBinding.observedUtc) { throw "Target capture '$captureKey' does not equal the target-process PNG FinalPath/FileId/link-count/length/LastWriteTime binding." }
