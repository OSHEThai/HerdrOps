...
function New-SyntheticAcceptanceArtifacts {
    ...
    $profile = Read-PackageProfile -Path (Join-Path $PSScriptRoot 'issue-44-package-profile.json')
...
function Invoke-AcceptancePreflight {
    ...
    if ([string]$upgradeArtifact.PackageVersion -cne $script:AcceptanceVersion.Substring(1)) {
        throw "Upgrade artifact package version is not $($script:AcceptanceVersion.Substring(1))."
    }
    if ($Mode -eq 'Live') {
        Add-AcceptancePreflightCheck -Name 'v1-target-version' -Status 'PASS' -Details 'Live upgrade artifact is bound to v1.0.0.'
    } else {
        Add-AcceptancePreflightCheck -Name 'v1-target-version' -Status 'PASS' -Details 'Exact v1.0 acceptance profile bound the synthetic upgrade artifact to the v1.0.0 target; the initial fixture is the explicitly named lower accepted 0.7.0 package.'
    }
...
function Get-DefaultFixtureRootForAcceptance {
    return Get-AcceptanceFullPath -Path (Join-Path $PSScriptRoot '..\..\tests\fixtures\v1.0\packaging')
}
