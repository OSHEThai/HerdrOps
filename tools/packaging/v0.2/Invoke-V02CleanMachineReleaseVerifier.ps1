...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($requestPath) -or $expectedRequestSha256 -cnotmatch '^[0-9A-F]{64}$') {
        throw 'The isolated automated lifecycle verifier request binding is missing or malformed.'
    }
    ...
    $required = @('reportPath','expectedSourceCommit','expectedSourceTree','engineSha256','verifierSha256','commonSha256','packagingCommonSha256','packageIdentityCommonSha256','rootPackagingCommonSha256','reportSha256','package')
    ...
    if ($actual.Count -ne $required.Count -or @($actual | Where-Object { $required -cnotcontains $_ }).Count -ne 0) {
        throw 'The isolated automated lifecycle verifier request has unexpected or missing properties.'
    }
    ...
    Assert-V02ReleaseVerifierEqual $commonSha256 $request.commonSha256 'Automated lifecycle common verifier SHA-256'
    ...
    Assert-V02ReleaseVerifierEqual $reportSha256 $request.reportSha256 'Automated lifecycle report held-input SHA-256'
    ...
    if ([string]$report.status -cne 'PASS') { throw 'Automated lifecycle report status must be PASS.' }
    if ([string]$report.mode -cne 'Live') { throw 'Automated lifecycle report mode must be Live.' }
    if ([string]$report.evidenceBoundary.evidenceClass -cne 'AutomatedLiveLifecycle') { throw 'Automated lifecycle evidence class must be AutomatedLiveLifecycle.' }
    if ([bool]$report.evidenceBoundary.creditGranted -ne $true) { throw 'Automated lifecycle credit must be granted.' }
    foreach ($phase in @('initial','final')) {
        ...
        foreach ($pair in @(
            ...
            Assert-V02ReleaseVerifierEqual $pair[0] $pair[1] "Automated lifecycle $($pair[2])"
        }
    }
    Assert-V02ReleaseVerifierEqual $report.profileId $request.package.profileId 'Automated lifecycle package profile'
    Assert-V02ReleaseVerifierEqual $report.bindings.referenceHostProfileSha256 $request.package.referenceHostProfileSha256 'Automated lifecycle reference-host profile'
    Assert-V02ReleaseVerifierEqual $report.bindings.rendererPolicySha256 $request.package.rendererPolicySha256 'Automated lifecycle renderer policy'
    Assert-V02ReleaseVerifierEqual $report.actor.operator.identity $report.machine.userScope 'Automated lifecycle operator/principal binding'

    $result = [pscustomobject][ordered]@{
        protocol = 'HerdrOps.V02IsolatedAutomatedLifecycleVerifierResult'
        version = 2
