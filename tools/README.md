...
# Verify the v0.6 Issue #31 Evaluation page gate without requiring a clean
# checkout. `-SkipBuild` skips only the build; dotnet test still runs.
./tools/Test-V06EvaluationPage.ps1 -Configuration Release
./tools/Test-V06EvaluationPage.ps1 -Configuration Release -SkipBuild

The v0.6 Issue #31 Evaluation page gate reports Static, Contract, Integration
and Synthetic evidence separately, pins the immutable `09-evaluation.png`
identity, records source/test and fresh TRX hashes, and validates fresh
reference-size, compact and implementation-named missing-score PNG evidence.
Actual Herdr Runtime, Independent Review and Release evidence remain explicitly
