...
./tools/Test-V02LiveRuntimeAcceptance.ps1 `
    ...
    -ExpectedSourceCommit $expectedSourceCommit `
    -ExpectedSourceTree $expectedSourceTree `
    -EvidenceRunNonce $evidenceRunNonce `
...
The composite gate binds production WPF captures to exact state hashes from the admitted Herdr Core trace and requires each Event phase to contain exactly one Core-labelled `pane.agent_status_changed` transition with matching accepted workspace/pane/status and App Agent evidence. It admits either a direct Event sequence or exactly one unlabelled connected snapshot reconciliation immediately before that Event; extra sequences, Events, topology changes or unrelated panes fail closed. It verifies Dashboard-close Widget continuity, waits for five seconds of an unchanged complete runtime fingerprint with SHA-256-chained append-only phase provenance, and then measures Widget latency plus combined Core/App idle resources. Dashboard and capture resources are released before managed large-object-heap cleanup, and weak references must show every tracked capture bitmap was collected; no native working-set trim is used. The 81 monotonic resource boundaries use `max(ordinal × 250 ms, previous observed boundary + 249 ms)`, and the independent validator enforces every ordinal floor, inclusive 249–500 ms adjacent deltas and an inclusive 20,000–20,250 ms final boundary; violations retain bounded diagnostics and receive `NoRuntimeCredit`. Under D-023, a future exact v0.2 candidate may use the 255 MiB (`267,386,880` byte) combined-maximum target only when producer and independent validator bind profile SHA-256 `96D01ED15A536F2DF50B59B43CFDEB3683DCE8667AE2E7BF6A96124182FE13A3`, schema SHA-256 `98AC6A2D823D88960A79299B7B20424FF60E9C5299D458A30AB9A42BE4FC0FB3`, the D-026 admitted host/OS, graphics-adapter/driver and installed-Herdr projection, and every pinned/recomputed or observed `candidatePolicy` leaf, including process-wide WPF `SoftwareOnly`; physical monitor, desktop DPI, power and thermal observations cannot grant or withhold v0.2 credit. Thai and English require separate complete report/capture sets with observed languages equal to their CLI requests, disjoint capture roots and an atomic same-candidate matrix manifest; that automated manifest requires a role-distinct Agent review and no ProductOwner test action. Prior 180 MiB failures remain `NoRuntimeCredit`, and v0.7 remains at 180 MiB. The gate also compares prelaunch and post-run App/Core executable hashes, emits a `NoRuntimeCredit` report on every terminating failure, fails closed outside an authorized Herdr pane, and relies on a separate automation Agent to control only the owned target Herdr session.
...
The v0.7 performance budget gate reports Static, Synthetic, and Contract evidence for non-runtime preparation (Issue #39). It enforces strict schema v0.7.0, source commit binding, candidate executable SHA-256 binding, reparse point and path traversal protections, p95 sample distribution recalculation, PID+StartUtc binding and PID reuse detection, no-native-trim waiver rules, and fault/unreconciled-state fail-closed behavior. Actual Herdr Runtime, 8-hour sustained soak execution, role-distinct Agent review, and Release Evidence remain explicitly NOT OBSERVED / NOT CLAIMED in this preparation slice. Human-only UAT is tracked in #161 and is not a gate.
