...
Assert-SourceContains -Path $runtimePackageBinding -Text 'New-Variable -Scope Script -Name V02GovernedPassingTestCount -Value 920 -Option Constant' -Description 'TRX preservation declares the governed exact current 920-test aggregate as a constant'
...
Assert-SourceContains -Path $runtimePackageBindingTests -Text '$script:V02GovernedPassingTestCount-ne 920' -Description 'Runtime binding tests fail visibly when the governed count drifts'
Assert-SourceContains -Path $runtimePackageBindingTests -Text "'stale 888-test aggregate fails closed and rolls back'" -Description 'Runtime binding tests reject and roll back the superseded 888-test aggregate'
Assert-SourceContains -Path $runtimePackageBindingTests -Text "'inconsistent total=passed=920 but notExecuted=1 reaches exact skipped guard'" -Description 'Runtime binding tests reach the explicit notExecuted guard'
...
Assert-SourceContains -Path $runtimePackageBindingTests -Text "'touched old 920 TRX set fails exact run-window guard'" -Description 'Runtime binding tests reject touched old test runs'
