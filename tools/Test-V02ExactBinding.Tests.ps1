...
Assert-SourceContains -Path $runtimePackageBindingTests -Text '$script:V02GovernedPassingTestCount-ne 926' -Description 'Runtime binding tests fail visibly when the governed count drifts'
...
Assert-SourceContains -Path $runtimePackageBindingTests -Text "'inconsistent total=passed=926 but notExecuted=1 reaches exact skipped guard'" -Description 'Runtime binding tests reach the explicit notExecuted guard'
...
Assert-SourceContains -Path $runtimePackageBindingTests -Text "'touched old 926 TRX set fails exact run-window guard'" -Description 'Runtime binding tests reject touched old test runs'
