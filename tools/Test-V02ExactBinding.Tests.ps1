...
Assert-SourceContains -Path $runtimePackageBinding -Text 'New-Variable -Scope Script -Name V02GovernedPassingTestCount -Value 919 -Option Constant' -Description 'TRX preservation declares the governed exact current 919-test aggregate as a constant'
...
Assert-SourceContains -Path $runtimePackageBindingTests -Text '$script:V02GovernedPassingTestCount-ne 919' -Description 'Runtime binding tests fail visibly when the governed count drifts'
...
Assert-SourceContains -Path $runtimePackageBindingTests -Text "'inconsistent total=passed=919 but notExecuted=1 reaches exact skipped guard'" -Description 'Runtime binding tests reach the explicit notExecuted guard'
...
Assert-SourceContains -Path $runtimePackageBindingTests -Text "'touched old 919 TRX set fails exact run-window guard'" -Description 'Runtime binding tests reject touched old test runs'
