...
function New-I10Fixture {
    ...
    $evidenceStartedUtc = [DateTimeOffset]::UtcNow.AddHours(-2)
    ...
    $rawOrders = @((New-PerformanceOrder -Order 'AB' -StartUtc $evidenceStartedUtc -OffsetSeconds 1),(New-PerformanceOrder -Order 'BA' -StartUtc $evidenceStartedUtc -OffsetSeconds 20)); $rawValue = [pscustomobject][ordered]@{ orders = @($rawOrders) }
    ...
    $rawCanonical=Get-I10CanonicalSha256 -Value $rawValue;$acquisitions=@();$acquisitionBase=$evidenceStartedUtc.AddSeconds(1)
    for($i=0;$i-lt24;$i++){$order=if($i-lt12){'AB'}else{'BA'};$within=$i%12;$pair=[int][Math]::Floor($within/2);$mode=if($order-ceq'AB'){if($within%2-eq0){'a'}else{'b'}}else{if($within%2-eq0){'b'}else{'a'}};$acquisitions+=[pscustomobject][ordered]@{sequenceNumber=$i;order=$order;isWarmup=($pair-eq0);repetitionOrdinal=if($pair-eq0){0}else{$pair-1};semanticMode=$mode;requestedMode=if($mode-ceq'a'){'Hardware'}else{'SoftwareOnly'};appProcessId=1000+$i;appStartUtc=$acquisitionBase.AddMilliseconds($i*10).ToString('O');appPath=$package.AppPath;appSha256=$package.AppSha256;coreProcessId=2000;coreStartUtc=$evidenceStartedUtc.ToString('O');corePath=$package.CorePath;coreSha256=$package.CoreSha256;serverProcessId=3000;serverStartUtc=$evidenceStartedUtc.ToString('O');serverPath='C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe';serverSha256=('D'*64);nativeProcessRenderMode=if($mode-ceq'a'){'Default'}else{'SoftwareOnly'};nativeTier=if($mode-ceq'a'){2}else{0};preFirstHwndProof=$true;observedUtc=$acquisitionBase.AddMilliseconds($i*10+5).ToString('O');boundary='PackagedCompatibilityPerformance-NativeTierComparator-NoPerFrameGpuOrRuntimeCredit'}}
    $sidecar=[pscustomobject][ordered]@{schemaVersion=2;evidenceClassification='PackagedCompatibilityPerformanceTelemetryBinding-NoRuntimeCredit';runNonce=$runNonce;source=[pscustomobject][ordered]@{commitSha=$commit;treeSha=$tree};session=[pscustomobject][ordered]@{kind='LocalConsole';name='Issue10PerformanceComparator';sessionId=1;transport='Physical';powerSource='AC';thermalState='Nominal';elevated=$false;userScope='SingleUser'};package=[pscustomobject][ordered]@{identitySha256=$package.ReceiptSha256;identityFileSha256=$package.IdentityFileSha256;profileFileSha256=$identityObject.profile.fileSha256;archiveSha256=$package.ArchiveSha256;manifestSha256=$package.ManifestSha256;appSha256=$package.AppSha256;coreSha256=$package.CoreSha256};rawSource=[pscustomobject][ordered]@{relativePath='performance/raw-observations.json';bytes=$rawHeld.Length;fileSha256=$rawHeld.Sha256;canonicalSha256=$rawCanonical};acquisitions=$acquisitions;evidenceBoundary=[pscustomobject][ordered]@{actualHerdrRuntime='NOT_OBSERVED';release='NOT_OBSERVED';creditGranted=$false}}
    $sidecarPath=Join-Path $root 'performance\performance-telemetry-binding.json';Write-FixtureJson $sidecarPath $sidecar;$sidecarHeld=Read-I10HeldFile -Path $sidecarPath -MaximumBytes 10000000 -Context 'fixture performance sidecar'
    $marker=[pscustomobject][ordered]@{schemaVersion=1;kind='issue10-performance-transaction-commit';runNonce=$runNonce;raw=[pscustomobject][ordered]@{fileName='raw-observations.json';bytes=$rawHeld.Length;sha256=$rawHeld.Sha256};binding=[pscustomobject][ordered]@{fileName='performance-telemetry-binding.json';bytes=$sidecarHeld.Length;sha256=$sidecarHeld.Sha256};creditGranted=$false};$markerPath=Join-Path $root 'performance\performance-commit.json';Write-FixtureJson $markerPath $marker;$markerHeld=Read-I10HeldFile -Path $markerPath -MaximumBytes 1000000 -Context 'fixture performance commit'
    $performanceProvenance=[pscustomobject][ordered]@{runNonce=$provenance.runNonce;candidate=$provenance.candidate;package=$provenance.package;profile=[pscustomobject][ordered]@{id=$identityObject.profile.id;relativePath=$identityObject.profile.relativePath;bytes=$identityObject.profile.bytes;fileSha256=$identityObject.profile.fileSha256;canonicalSha256=$identityObject.profile.canonicalSha256};referenceHost=$identityObject.referenceHost;renderer=[pscustomobject][ordered]@{policy='software-only-process-wide';wpfProcessRenderMode='SoftwareOnly';policySha256='1D37C9C39449556EB30F9AB5B734F0C5411CF4203321AF0B238993D017229E92'};session=[pscustomobject][ordered]@{kind='LocalConsole';name='Issue10PerformanceComparator';sessionId=1;transport='Physical';powerSource='AC';thermalState='Nominal';elevated=$false;userScope='SingleUser'};performanceTelemetryBinding=[pscustomobject][ordered]@{relativePath='performance/performance-telemetry-binding.json';bytes=$sidecarHeld.Length;fileSha256=$sidecarHeld.Sha256;canonicalSha256=(Get-I10CanonicalSha256 -Value $sidecar)};performanceTransactionCommit=[pscustomobject][ordered]@{relativePath='performance/performance-commit.json';bytes=$markerHeld.Length;fileSha256=$markerHeld.Sha256;canonicalSha256=(Get-I10CanonicalSha256 -Value $marker)}}
    $performanceValue = [pscustomobject][ordered]@{ schemaVersion=4; provenance = $performanceProvenance; rawSource = [pscustomobject][ordered]@{ relativePath = 'performance/raw-observations.json'; bytes = $rawHeld.Length; fileSha256 = $rawHeld.Sha256; canonicalSha256 = $rawCanonical }; orders = @($rawOrders); aggregateStatus = 'PASS' }
    ...
    foreach ($language in @('Thai','English')) {
        ...
        $widgetValue = [pscustomobject][ordered]@{
            ...
            Source = [pscustomobject][ordered]@{ CommitSha = $commit; TreeSha = $tree }
            Bindings = [pscustomobject][ordered]@{ GateReportSha256 = (Get-I10Lines -Path $gatePaths[$language]).Hash; AppRuntimeReportSha256 = (Read-I10HeldFile -Path $appReports[$language] -MaximumBytes 1000000 -Context 'app').Sha256; CoreRuntimeReportSha256 = (Read-I10HeldFile -Path $coreReports[$language] -MaximumBytes 1000000 -Context 'core').Sha256; PackageIdentityFileSha256 = $package.IdentityFileSha256; PackageIdentityReceiptSha256 = $package.ReceiptSha256; PackageArchiveSha256 = $package.ArchiveSha256; PackageManifestSha256 = $package.ManifestSha256; AppSha256 = $package.AppSha256; CoreSha256 = $package.CoreSha256; HerdrExecutableSha256 = $herdrHash; PerformanceReceiptSha256 = (Read-I10HeldFile -Path $performancePath -MaximumBytes 10000000 -Context 'performance').Sha256; PerformanceTelemetryBindingSha256=$sidecarHeld.Sha256;PerformanceTransactionCommitSha256=$markerHeld.Sha256; ControlSessionIdentity = $control; TargetSessionIdentity = $target }
    ...
    [pscustomobject][ordered]@{ Root = $root; RepositoryRoot = $repositoryRoot; Commit = $commit; Tree = $tree; RunNonce = $runNonce; EvidenceStartedUtc = $evidenceStartedUtc; Package = $package; PackageBinding = $package; PerformancePath = $performancePath; PerformanceSidecarPath=$sidecarPath;PerformanceCommitPath=$markerPath; ThaiWidget = $widgetReportByLanguage['Thai']; EnglishWidget = $widgetReportByLanguage['English']; ThaiGate = $gatePaths['Thai']; EnglishGate = $gatePaths['English']; OutputPath = (Join-Path $root 'candidate\issue10-runtime-candidate.json') }
}

function Invoke-Fixture {
    param([Parameter(Mandatory = $true)]$Fixture,[string]$OutputPath = $Fixture.OutputPath)
    return Invoke-I10Issue10Acceptance -EvidenceRoot $Fixture.Root -ThaiWidgetReportPath $Fixture.ThaiWidget -EnglishWidgetReportPath $Fixture.EnglishWidget -ThaiRuntimeGatePath $Fixture.ThaiGate -EnglishRuntimeGatePath $Fixture.EnglishGate -PerformanceReceiptPath $Fixture.PerformancePath -ExpectedSourceCommit $Fixture.Commit -ExpectedSourceTree $Fixture.Tree -PackageBinding $Fixture.PackageBinding -OutputPath $OutputPath -RunNonce $Fixture.RunNonce -EvidenceStartedUtc $Fixture.EvidenceStartedUtc -FixtureMode
}

function Update-PerformanceSidecarFixture {
    param($Fixture,[scriptblock]$Mutation)
    $sidecar=(Read-I10StrictJson -Path $Fixture.PerformanceSidecarPath -Context 'mutated sidecar').Value;&$Mutation $sidecar;Write-FixtureJson $Fixture.PerformanceSidecarPath $sidecar
    $sidecarHeld=Read-I10HeldFile -Path $Fixture.PerformanceSidecarPath -MaximumBytes 10000000 -Context 'mutated sidecar bytes'
    $marker=(Read-I10StrictJson -Path $Fixture.PerformanceCommitPath -Context 'mutated marker').Value;$marker.binding.bytes=$sidecarHeld.Length;$marker.binding.sha256=$sidecarHeld.Sha256;Write-FixtureJson $Fixture.PerformanceCommitPath $marker
    $markerHeld=Read-I10HeldFile -Path $Fixture.PerformanceCommitPath -MaximumBytes 1000000 -Context 'mutated marker bytes'
    $receipt=(Read-I10StrictJson -Path $Fixture.PerformancePath -Context 'mutated receipt').Value
    $receipt.provenance.performanceTelemetryBinding.bytes=$sidecarHeld.Length;$receipt.provenance.performanceTelemetryBinding.fileSha256=$sidecarHeld.Sha256;$receipt.provenance.performanceTelemetryBinding.canonicalSha256=Get-I10CanonicalSha256 -Value $sidecar
    $receipt.provenance.performanceTransactionCommit.bytes=$markerHeld.Length;$receipt.provenance.performanceTransactionCommit.fileSha256=$markerHeld.Sha256;$receipt.provenance.performanceTransactionCommit.canonicalSha256=Get-I10CanonicalSha256 -Value $marker
    Write-FixtureJson $Fixture.PerformancePath $receipt
}

try {
    $valid = New-I10Fixture
    $result = Invoke-Fixture -Fixture $valid
    if ([long]$result.Candidate.SchemaVersion-ne4-or$null-ne$result.Candidate.PSObject.Properties['Soak']-or$null-ne$result.Candidate.EvidenceBoundary.PSObject.Properties['Human']-or[string]$result.Candidate.EvidenceClassification -cne 'Issue10RuntimeCandidate' -or [string]$result.Candidate.EvidenceBoundary.RuntimeInput -cne 'SYNTHETIC_FIXTURE_BOUND' -or [string]$result.Candidate.EvidenceBoundary.Runtime -cne 'NOT_OBSERVED' -or [string]$result.Candidate.EvidenceBoundary.Release -cne 'NOT_OBSERVED' -or [bool]$result.Candidate.EvidenceBoundary.CreditGranted -or [string]$result.Candidate.RunNonce -cne $valid.RunNonce) { throw 'Valid Issue #10 fixture crossed the v4 evidence boundary or retained removed fields.' }
    Pass-Test 'valid bilingual candidate remains RuntimeCandidate with dual-hash and nonce binding'
    if(Get-Command Test-Json -ErrorAction SilentlyContinue){$candidateSchema=Join-Path $PSScriptRoot 'issue10-runtime-candidate.schema.json';$missingPerformanceField=($result.Candidate|ConvertTo-Json -Depth 100|ConvertFrom-Json -DateKind String);$missingPerformanceField.Performance.PSObject.Properties.Remove('TelemetryBindingPath');if(($missingPerformanceField|ConvertTo-Json -Depth 100|Test-Json -SchemaFile $candidateSchema -ErrorAction SilentlyContinue)){throw 'Issue #10 schema admitted a missing TelemetryBindingPath.'};Pass-Test 'candidate schema rejects missing performance authority field';$extraPerformanceField=($result.Candidate|ConvertTo-Json -Depth 100|ConvertFrom-Json -DateKind String);$extraPerformanceField.Performance|Add-Member unexpectedAuthority 'forbidden';if(($extraPerformanceField|ConvertTo-Json -Depth 100|Test-Json -SchemaFile $candidateSchema -ErrorAction SilentlyContinue)){throw 'Issue #10 schema admitted an extra performance authority field.'};Pass-Test 'candidate schema rejects extra performance authority field'}
    ...
    $missingSidecar=New-I10Fixture;$missingSidecarValue=(Read-I10StrictJson -Path $missingSidecar.PerformancePath -Context 'missing sidecar').Value;$missingSidecarValue.provenance.PSObject.Properties.Remove('performanceTelemetryBinding');Write-FixtureJson $missingSidecar.PerformancePath $missingSidecarValue;Assert-Throws {Invoke-Fixture $missingSidecar} 'missing performance sidecar provenance fails closed' 'performanceTelemetryBinding'
    $tamperedSidecar=New-I10Fixture;[IO.File]::AppendAllText($tamperedSidecar.PerformanceSidecarPath,' ');Assert-Throws {Invoke-Fixture $tamperedSidecar} 'tampered performance sidecar bytes fail closed' 'sidecar.*binding|canonical'
    $offsetSidecar=New-I10Fixture;Update-PerformanceSidecarFixture $offsetSidecar {param($value)$time=[DateTimeOffset]::Parse([string]$value.acquisitions[0].appStartUtc);$value.acquisitions[0].appStartUtc=$time.ToOffset([TimeSpan]::FromHours(7)).ToString('O')};Assert-Throws {Invoke-Fixture $offsetSidecar} 'nonzero-offset sidecar process timestamp fails exact UTC guard' 'exact round-trip UTC'
    $stringSequenceSidecar=New-I10Fixture;Update-PerformanceSidecarFixture $stringSequenceSidecar {param($value)$value.acquisitions[0].sequenceNumber='0'};Assert-Throws {Invoke-Fixture $stringSequenceSidecar} 'performance sidecar string sequenceNumber fails native integer guard' 'native JSON integer'
    $stringPreHwndSidecar=New-I10Fixture;Update-PerformanceSidecarFixture $stringPreHwndSidecar {param($value)$value.acquisitions[0].preFirstHwndProof='true'};Assert-Throws {Invoke-Fixture $stringPreHwndSidecar} 'performance sidecar string preFirstHwndProof fails native boolean guard' 'native JSON boolean'
    $stringCreditSidecar=New-I10Fixture;Update-PerformanceSidecarFixture $stringCreditSidecar {param($value)$value.evidenceBoundary.creditGranted='false'};Assert-Throws {Invoke-Fixture $stringCreditSidecar} 'performance sidecar string creditGranted fails native boolean guard' 'native JSON boolean'
    $transplantedCommit=New-I10Fixture;$commitValue=(Read-I10StrictJson -Path $transplantedCommit.PerformanceCommitPath -Context 'transplanted commit').Value;$commitValue.runNonce=('0'*32);Write-FixtureJson $transplantedCommit.PerformanceCommitPath $commitValue;$commitHeld=Read-I10HeldFile -Path $transplantedCommit.PerformanceCommitPath -MaximumBytes 1000000 -Context 'transplanted commit';$performanceValue=(Read-I10StrictJson -Path $transplantedCommit.PerformancePath -Context 'transplanted performance').Value;$performanceValue.provenance.performanceTransactionCommit.bytes=$commitHeld.Length;$performanceValue.provenance.performanceTransactionCommit.fileSha256=$commitHeld.Sha256;$performanceValue.provenance.performanceTransactionCommit.canonicalSha256=Get-I10CanonicalSha256 $commitValue;Write-FixtureJson $transplantedCommit.PerformancePath $performanceValue;Assert-Throws {Invoke-Fixture $transplantedCommit} 'transplanted performance commit nonce fails closed' 'transaction commit does not bind|runNonce'
    $stringCommitCredit=New-I10Fixture;$stringCommitValue=(Read-I10StrictJson -Path $stringCommitCredit.PerformanceCommitPath -Context 'string commit credit').Value;$stringCommitValue.creditGranted='false';Write-FixtureJson $stringCommitCredit.PerformanceCommitPath $stringCommitValue;$stringCommitHeld=Read-I10HeldFile -Path $stringCommitCredit.PerformanceCommitPath -MaximumBytes 1000000 -Context 'string commit credit';$stringCommitReceipt=(Read-I10StrictJson -Path $stringCommitCredit.PerformancePath -Context 'string commit receipt').Value;$stringCommitReceipt.provenance.performanceTransactionCommit.bytes=$stringCommitHeld.Length;$stringCommitReceipt.provenance.performanceTransactionCommit.fileSha256=$stringCommitHeld.Sha256;$stringCommitReceipt.provenance.performanceTransactionCommit.canonicalSha256=Get-I10CanonicalSha256 $stringCommitValue;Write-FixtureJson $stringCommitCredit.PerformancePath $stringCommitReceipt;Assert-Throws {Invoke-Fixture $stringCommitCredit} 'performance commit string creditGranted fails native boolean guard' 'native JSON boolean'
    ...
    $legacyPerformance=New-I10Fixture;$legacyValue=(Read-I10StrictJson $legacyPerformance.PerformancePath 'legacy performance').Value;$legacyValue.schemaVersion=3;Write-FixtureJson $legacyPerformance.PerformancePath $legacyValue;Assert-Throws {Invoke-Fixture $legacyPerformance} 'legacy performance receipt cannot close v4' 'schema-v4'
    $soakShapedReceipt=New-I10Fixture;$soakReceipt=(Read-I10StrictJson $soakShapedReceipt.PerformancePath 'legacy soak-shaped receipt').Value;$soakReceipt|Add-Member soakBins @();Write-FixtureJson $soakShapedReceipt.PerformancePath $soakReceipt;Assert-Throws {Invoke-Fixture $soakShapedReceipt} 'legacy performance soakBins cannot close v4' 'unknown, missing|exactly'
    ...
    $postMoveRace = New-I10Fixture; $postMoveHardlink = Join-Path $postMoveRace.Package.PackageRoot 'identity-post-move-hardlink'; $postMoveState = [pscustomobject]@{ HookReached = $false; HardlinkCreated = $false }; Set-I10TestHook -Hook { param($name,$transaction,$data); if ($name -ceq 'AfterPublishedCandidateVerification') { $postMoveState.HookReached = $true; New-Item -ItemType HardLink -Path $postMoveHardlink -Target $postMoveRace.Package.IdentityPath -Force | Out-Null; $postMoveState.HardlinkCreated = $true } }; try { Assert-Throws { Invoke-Fixture -Fixture $postMoveRace } 'post-move input hardlink reaches the final transaction guard' 'NumberOfLinks changed|transaction after published candidate verification' } finally { Set-I10TestHook -Hook $null; if (Test-Path -LiteralPath $postMoveHardlink) { Remove-Item -LiteralPath $postMoveHardlink -Force } }; if (-not $postMoveState.HookReached -or -not $postMoveState.HardlinkCreated) { throw 'Post-move publication hook did not create the hostile admitted-input hardlink.' }; if (Test-Path -LiteralPath $postMoveRace.OutputPath) { throw 'Post-move rejection left a transaction-owned published candidate.' }; $postMoveStages = @(Get-ChildItem -LiteralPath $postMoveRace.Root -Filter '.issue10-stage-*.json' -File -ErrorAction SilentlyContinue); if ($postMoveStages.Count -ne 0) { throw 'Post-move rejection left a staging candidate.' }; Pass-Test 'post-move input hardlink rejects before authority return and cleans owned publication'

    Pass-Test 'fixture path never starts Herdr, HerdrOps, or an application process'
    [pscustomobject][ordered]@{ EvidenceClassification = 'SyntheticVerifierSelftest'; PositiveCases = $script:Passed; NegativeCases = ($script:Passed - 2); Runtime = 'NOT_OBSERVED'; Release = 'NOT_OBSERVED'; CreditGranted = $false }
}
catch {
    $script:Failed++
    throw
