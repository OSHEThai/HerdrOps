...
function New-I10Fixture {
    ...
    $sidecar=[pscustomobject][ordered]@{schemaVersion=1;evidenceClassification='PackagedCompatibilityPerformanceTelemetryBinding-NoRuntimeCredit';runNonce=$runNonce;source=[pscustomobject][ordered]@{commitSha=$commit;treeSha=$tree};package=[pscustomobject][ordered]@{identitySha256=$package.ReceiptSha256;identityFileSha256=$package.IdentityFileSha256;profileFileSha256=$identityObject.profile.fileSha256;archiveSha256=$package.ArchiveSha256;manifestSha256=$package.ManifestSha256;appSha256=$package.AppSha256;coreSha256=$package.CoreSha256};rawSource=[pscustomobject][ordered]@{relativePath='performance/raw-observations.json';bytes=$rawHeld.Length;fileSha256=$rawHeld.Sha256;canonicalSha256=$rawCanonical};acquisitions=$acquisitions;evidenceBoundary=[pscustomobject][ordered]@{actualHerdrRuntime='NOT_OBSERVED';humanReview='NOT_OBSERVED';release='NOT_OBSERVED';creditGranted=$false}}
    ...
    $performanceValue = [pscustomobject][ordered]@{ provenance = $performanceProvenance; rawSource = [pscustomobject][ordered]@{ relativePath = 'performance/raw-observations.json'; bytes = $rawHeld.Length; fileSha256 = $rawHeld.Sha256; canonicalSha256 = $rawCanonical }; orders = @($rawOrders); soakBins = @($rawBins); aggregateStatus = 'PASS' }
