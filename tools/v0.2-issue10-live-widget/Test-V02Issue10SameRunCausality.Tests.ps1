...
try{
    $performance=Join-Path $fixture 'performance.json';$raw=Join-Path $fixture 'raw.json';$telemetryBinding=Join-Path $fixture 'telemetry-binding.json';$transactionCommit=Join-Path $fixture 'transaction-commit.json'
    foreach($path in @($performance,$raw,$telemetryBinding,$transactionCommit)){[IO.File]::WriteAllText($path,"{}`n",(New-Object Text.UTF8Encoding($false)))}
    $fullInputs=@('-Issue10PerformanceReceiptPath',$performance,'-Issue10PerformanceRawSourcePath',$raw,'-Issue10PerformanceTelemetryBindingPath',$telemetryBinding,'-Issue10PerformanceTransactionCommitPath',$transactionCommit)
    ...
    try{
        ...
        if($junctionCreated){
            ...
            $junctionInputs=@('-Issue10PerformanceReceiptPath',$junctionReceipt,'-Issue10PerformanceRawSourcePath',$raw,'-Issue10PerformanceTelemetryBindingPath',$telemetryBinding,'-Issue10PerformanceTransactionCommitPath',$transactionCommit)
    ...
    if(Test-Path -LiteralPath $appExecutable -PathType Leaf){
        ...
        $artifactFiles=@{};foreach($name in @('gate.txt','core.json','identity.json','archive.zip','package.json','package-app.exe','package-core.exe','performance.json','performance-raw.json','performance-telemetry-binding.json','performance-transaction-commit.json','herdr.exe')){$path=Join-Path $fixture $name;&$writeUtf8 $path ("owned-$name`n");$artifactFiles[$name]=$path}
