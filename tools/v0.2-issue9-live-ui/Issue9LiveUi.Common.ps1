...
function Assert-I9Gate {
    ...
    if(-not[DateTimeOffset]::TryParse((Get-I9GateValue $Gate 'GeneratedUtc' $context),[Globalization.CultureInfo]::InvariantCulture,[Globalization.DateTimeStyles]::RoundtripKind,[ref]$generatedUtc)){throw "$context GeneratedUtc is invalid."}
    $now=[DateTimeOffset]::UtcNow;$generatedUtc=$generatedUtc.ToUniversalTime();if($generatedUtc-gt$now-or$generatedUtc-lt$now.AddMinutes(-$script:I9MaximumAgeMinutes)){throw "$context is outside the bounded fresh review window."}
