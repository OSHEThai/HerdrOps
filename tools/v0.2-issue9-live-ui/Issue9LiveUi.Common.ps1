...
function Assert-I9MatrixCandidate {
    ...
    $payload = $doc.Value.Payload; Assert-I9ExactProperties $payload @('GeneratedUnixTimeMilliseconds','IndependentHumanReview','ReleaseCredit','Binding','Runs') 'matrix payload'; Assert-I9String $payload.IndependentHumanReview 'matrix payload review' 'NOT_OBSERVED' | Out-Null; Assert-I9False $payload.ReleaseCredit 'matrix payload release credit'
    ...
    foreach ($pair in @(@('Thai',$Thai),@('English',$English))) { $lang = [string]$pair[0]; if (-not $byLanguage.ContainsKey($lang)) { throw "Matrix candidate is missing $lang." }; $run = $byLanguage[$lang]; $actual = $pair[1]; foreach ($name in @('EvidenceDirectory','GateReportSha256','AppRuntimeReportSha256','CoreRuntimeReportSha256','SourceCommit','SourceTree','PackageIdentityReceiptSha256')) { $null = Get-I9Prop $run $name "matrix $lang run" }; if ([string]$run.EvidenceDirectory -cne $actual.Root -or [string]$run.GateReportSha256 -cne $actual.GateSha256 -or [string]$run.AppRuntimeReportSha256 -cne $actual.AppSha256 -or [string]$run.CoreRuntimeReportSha256 -cne $actual.CoreSha256 -or [string]$run.SourceCommit -cne $ExpectedCommit -or [string]$run.SourceTree -cne $ExpectedTree -or [string]$run.PackageIdentityReceiptSha256 -cne $Package.ReceiptSha256) { throw "Matrix $lang run is not bound to the exact runtime leg." } }
    return [pscustomobject][ordered]@{ Path = $doc.Path; FileSha256 = $doc.Sha256; PayloadSha256 = $payloadHash }
}
...
function Assert-I9DerivedMatrix {
    ...
    try {
        $null = @(& $matrixScript -ThaiEvidenceDirectory $ThaiRuntimeRoot -EnglishEvidenceDirectory $EnglishRuntimeRoot -PackageIdentityPath $PackageIdentityPath -PackageArchivePath $PackageArchivePath -ExtractedPackageRoot $PackageRoot -RepositoryRoot $RepositoryRoot -PackageProfilePath $profile -OutputPath $derivedPath)
