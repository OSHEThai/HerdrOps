...
try {
    ...
    function New-I9Gate {
        param([Parameter(Mandatory)][string]$RuntimeRoot, [Parameter(Mandatory)][ValidateSet('Thai','English')][string]$Language, [Parameter(Mandatory)][string]$AppSha, [Parameter(Mandatory)][string]$CoreSha)
    ...
    function New-I9RuntimeLeg {
        ...
        $gatePath = New-I9Gate $RuntimeRoot $Language $appHash $coreHash
        return [pscustomobject]@{ Root = [IO.Path]::GetFullPath($RuntimeRoot); GatePath = $gatePath; AppHash = $appHash; CoreHash = $coreHash; GateHash = (Read-I9HeldFile $gatePath).Sha256 }
    }
