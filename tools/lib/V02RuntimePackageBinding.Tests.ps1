...
Test-Case 'governed passing-test count is exact current aggregate' { if($script:V02GovernedPassingTestCount-ne 920){throw "Governed passing-test count drifted: $script:V02GovernedPassingTestCount"} }
...
try {
    ...
    Write-Trx (Join-Path $results 'test4.trx') $script:V02GovernedTestAssemblyFileNames[3] $runIds[3] 148 147 1
    ...
    Write-Trx (Join-Path $results 'test4.trx') $script:V02GovernedTestAssemblyFileNames[3] $runIds[3] 148 148 0 1
    Test-Case 'inconsistent total=passed=920 but notExecuted=1 reaches exact skipped guard' { Assert-ExactTrxFailure 'Fresh test counters contain skipped/notExecuted tests: skipped=0 notExecuted=1' { Save-V02FreshTrxEvidence $results $started $evidence } }
    Write-Trx (Join-Path $results 'test4.trx') $script:V02GovernedTestAssemblyFileNames[3] $runIds[3] 148 148 0 0 1
    Test-Case 'inconsistent total=passed=920 but skipped=1 reaches exact skipped guard' { Assert-ExactTrxFailure 'Fresh test counters contain skipped/notExecuted tests: skipped=1 notExecuted=0' { Save-V02FreshTrxEvidence $results $started $evidence } }
    ...
    Test-Case 'touched old 920 TRX set fails exact run-window guard' { Assert-ExactTrxFailure 'TRX TestRun is outside the exact invocation window: test1.trx' { Save-V02FreshTrxEvidence $results $started $evidence } }
