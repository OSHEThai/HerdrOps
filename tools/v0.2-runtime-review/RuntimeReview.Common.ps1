...
function Assert-V02RuntimeReviewSelectionReceipt {
    ...
    $r=$doc.Value;Assert-V02RuntimeReviewExactProperties $r @('SchemaVersion','InvocationStartedUtc','SelectionUpperBoundUtc','FileCount','Total','Passed','Failed','NotExecuted','Skipped','Files') "$Context selection receipt";if([int64]$r.SchemaVersion-ne2-or[int64]$r.FileCount-ne4-or[int64]$r.Total-ne920-or[int64]$r.Passed-ne920-or[int64]$r.Failed-ne0-or[int64]$r.NotExecuted-ne0-or[int64]$r.Skipped-ne0-or@($r.Files).Count-ne4){throw "$Context selection receipt counters are not governed and all-passing."};$started=[DateTimeOffset]$r.InvocationStartedUtc;$upper=[DateTimeOffset]$r.SelectionUpperBoundUtc;if($started-ge$upper){throw "$Context selection receipt chronology is invalid."}
    ...
    if($seen.Count-ne4-or$sum-ne920){throw "$Context selected TRX aggregate is not the exact four-file 920/920 set."}
}
