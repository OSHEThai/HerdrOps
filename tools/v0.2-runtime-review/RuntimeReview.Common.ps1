...
function ConvertFrom-V02RuntimeReviewNativeSessionReference {
    ...
    Assert-V02RuntimeReviewExactProperties $native @('agent','kind','source','value') $Context
    foreach($name in @('agent','kind','source','value')){Assert-V02RuntimeReviewString $native.PSObject.Properties[$name].Value "$Context $name"|Out-Null}
...
function Assert-V02RuntimeReviewSelectionReceipt {
    ...
    $r=$doc.Value;Assert-V02RuntimeReviewExactProperties $r @('SchemaVersion','InvocationStartedUtc','SelectionUpperBoundUtc','FileCount','Total','Passed','Failed','NotExecuted','Skipped','Files') "$Context selection receipt";if([int64]$r.SchemaVersion-ne2-or[int64]$r.FileCount-ne4-or[int64]$r.Total-ne926-or[int64]$r.Passed-ne926-or[int64]$r.Failed-ne0-or[int64]$r.NotExecuted-ne0-or[int64]$r.Skipped-ne0-or@($r.Files).Count-ne4){throw "$Context selection receipt counters are not governed and all-passing."};$started=[DateTimeOffset]$r.InvocationStartedUtc;$upper=[DateTimeOffset]$r.SelectionUpperBoundUtc;if($started-ge$upper){throw "$Context selection receipt chronology is invalid."}
    ...
    if($seen.Count-ne4-or$sum-ne926){throw "$Context selected TRX aggregate is not the exact four-file 926/926 set."}
}
...
function Invoke-V02RuntimeReviewVerificationCore {
    ...
    if($thaiGateIdentity.RunNonce-cne$englishGateIdentity.RunNonce){throw 'Thai and English EvidenceRunNonce values must be identical for one bilingual acceptance transaction.'}
    ...
    foreach ($name in @('AcceptanceControlSession','TargetAgentLabSession','AcceptanceControlSocketPath','TargetAgentLabSocketPath','TargetAgentSessionReference','TargetAgentSessionReferenceEvidenceSource','TargetAgentSessionReferenceObservableByGate','TargetAgentSessionReferenceBoundary','AcceptanceControlServerIdentity')) { if ((Get-V02RuntimeReviewGateValue $thaiGate $name 'Thai session') -cne (Get-V02RuntimeReviewGateValue $englishGate $name 'English session')) { throw "Thai and English $name identities are not exact." } }
    ...
    if (-not $FixtureMode) { Assert-V02RuntimeReviewSourceExact $RepositoryRoot $ExpectedSourceCommit $ExpectedSourceTree }
    $nativeSession=ConvertFrom-V02RuntimeReviewNativeSessionReference (Get-V02RuntimeReviewGateValue $thaiGate 'TargetAgentSessionReference' 'Thai gate') 'Thai gate native session'
