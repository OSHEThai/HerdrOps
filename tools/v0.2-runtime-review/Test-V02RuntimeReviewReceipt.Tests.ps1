...
function New-RRSelectionReceipt {
    ...
    $directory=Join-Path $Evidence 'test-results';$entries=@();$names=@('HerdrOps.UnitTests.trx','HerdrOps.ContractTests.trx','HerdrOps.IntegrationTests.trx','HerdrOps.RuntimeTests.trx');$counts=@(191,97,484,154);for($index=0;$index-lt$names.Count;$index++){$name=$names[$index];$count=$counts[$index];$path=Join-Path $directory $name;$runId=[guid]::NewGuid().ToString('D');$assembly=$name-replace'\.trx$','.dll';$xml="<TestRun id=`"$runId`"><Times start=`"$($Utc.ToString('O'))`" finish=`"$($Utc.AddSeconds(1).ToString('O'))`"/><TestDefinitions><UnitTest storage=`"$assembly`"/></TestDefinitions><ResultSummary><Counters total=`"$count`" passed=`"$count`" failed=`"0`" notExecuted=`"0`" skipped=`"0`"/></ResultSummary></TestRun>";Write-RRFixtureText $path $xml;$file=Get-RRFixtureFile $path;$entries+=[ordered]@{Name=$name;SourceName=$name;Bytes=$file.Bytes;Sha256=$file.Sha256;LastWriteUtc=$Utc.ToString('O');TestRunId=$runId;RunStartedUtc=$Utc.ToString('O');RunFinishedUtc=$Utc.AddSeconds(1).ToString('O');TestAssemblyFileName=$assembly;Total=$count;Passed=$count;Failed=0;NotExecuted=0;Skipped=0}}
    $path=Join-Path $directory 'selection-receipt.json';Write-RRFixtureJson $path ([ordered]@{SchemaVersion=2;InvocationStartedUtc=$Utc.ToString('O');SelectionUpperBoundUtc=$Utc.AddSeconds(2).ToString('O');FileCount=4;Total=926;Passed=926;Failed=0;NotExecuted=0;Skipped=0;Files=$entries});return $path
}
...
function New-RRFixture {
    ...
    foreach ($language in @('Thai', 'English')) {
        ...
        $gateLines=@($gateLines|ForEach-Object{([string]$_).Replace('TargetAgentSessionReference: agent-lab-session-1',"TargetAgentSessionReference: $nativeSessionReference").Replace('TargetAgentSessionReferenceEvidenceSource: fixture','TargetAgentSessionReferenceEvidenceSource: HerdrCliAgentMetadata').Replace('TargetAgentSessionReferenceObservableByGate: fixture','TargetAgentSessionReferenceObservableByGate: true').Replace('TargetAgentSessionReferenceBoundary: fixture','TargetAgentSessionReferenceBoundary: The gate directly observed and exact-bound the same structured native Agent session through Herdr CLI metadata before restart, at reconnect, and through completion.').Replace('The native target Agent/session reference is operator attestation because the gate cannot independently observe that client-owned session identity.','The gate directly observed and exact-bound the same structured native Agent session through Herdr CLI metadata before restart, at reconnect, and through completion.')})
...
try {
    ...
    if([int64]$candidateDocument.Value.SchemaVersion-ne2-or[string]$candidateDocument.Value.Sessions.Target.EvidenceSource-cne'HerdrCliAgentMetadata'-or-not[bool]$candidateDocument.Value.Sessions.Target.ObservableByGate){throw 'Emitted successor candidate did not preserve gate-observed native session authority.'}
    ...
    $fixture=New-RRFixture;$fixtures+=$fixture;$gatePath=Join-Path $fixture.Thai 'gate-report.txt';Set-RRGateField $gatePath 'TargetAgentSessionReferenceEvidenceSource' 'OperatorAttestation';Sync-RRFixtureLeg $fixture Thai
    Assert-RRFailure 'legacy operator target-session attestation' {Invoke-RRFixture $fixture|Out-Null} 'HerdrCliAgentMetadata|gate-observed authority'

    $fixture=New-RRFixture;$fixtures+=$fixture;$gatePath=Join-Path $fixture.English 'gate-report.txt';Set-RRGateField $gatePath 'TargetAgentSessionReferenceObservableByGate' 'false';Sync-RRFixtureLeg $fixture English
    Assert-RRFailure 'unobservable target-session reference' {Invoke-RRFixture $fixture|Out-Null} 'gate-observed authority'
    ...
    Assert-RRFailure 'receipt-only redistributed 926 counters' {Invoke-RRFixture $fixture|Out-Null} 'not independently derived'
