...
try {
    ...
    $p = Start-Process -FilePath $cliExe -ArgumentList $cliArgs -WindowStyle Hidden -Wait -PassThru -RedirectStandardOutput $cliStdout -RedirectStandardError $cliStderr
    ...
    if ($missingNonceError -notmatch '(?s)ReviewRunNonce.*mandatory parameter|mandatory parameter.*ReviewRunNonce') {
        throw "CLI invocation without ReviewRunNonce reached the wrong guard: $missingNonceError"
    }
    ...
    if ($p.ExitCode -eq 0 -or $cliError -notmatch 'Identity profile leaf values do not match the stable preparation profile') {
        $cliOutput = if (Test-Path -LiteralPath $cliStdout) { [IO.File]::ReadAllText($cliStdout) } else { '' }
        throw "CLI invocation with ReviewRunNonce did not advance to the expected production package guard. Exit code: $($p.ExitCode). stderr: $cliError stdout: $cliOutput"
    }
