...
function New-RRProgressEntry {
    ...
    $entry=[ordered]@{Ordinal=$Ordinal;Phase=$Phase;ObservedUtc=$Utc.ToString('O');Sequence=$Sequence;IsCoreConnected=$Connected;IsLive=$Connected;RuntimeStatus=$Status;LastTransitionUtc=$Utc.ToString('O');LastAcceptedStateUtc=$Utc.ToString('O');ConnectionEpoch=1;BootstrapCount=1;EventCount=$Sequence;DisconnectCount=$(if($Ordinal-ge5){1}else{0});ReconciliationCount=$Sequence;StateSha256=$StateSha;PreviousEntrySha256=$Previous;CanonicalPayload='';EntrySha256=''}
...
function New-RRSelectionReceipt {
    ...
    $directory=Join-Path $Evidence 'test-results';$entries=@();foreach($name in @('HerdrOps.UnitTests.trx','HerdrOps.ContractTests.trx','HerdrOps.IntegrationTests.trx','HerdrOps.RuntimeTests.trx')){$path=Join-Path $directory $name;Write-RRFixtureText $path "trx-$name`n";$file=Get-RRFixtureFile $path;$entries+=[ordered]@{Name=$name;SourceName=$name;Bytes=$file.Bytes;Sha256=$file.Sha256;LastWriteUtc=$Utc.ToString('O');TestRunId=([guid]::NewGuid().ToString());RunStartedUtc=$Utc.ToString('O');RunFinishedUtc=$Utc.AddSeconds(1).ToString('O');TestAssemblyFileName=($name-replace'\.trx$','.dll');Total=222;Passed=222;Failed=0;NotExecuted=0;Skipped=0}}
...
try {
    ...
    $fixture=New-RRFixture;$fixtures+=$fixture;$appPath=Join-Path $fixture.Thai 'app-runtime.json';$appDoc=(Read-V02RuntimeReviewStrictJsonFile $appPath 'disconnect hash').Value;$appDoc.DisconnectStateSha256='9'*64;Write-RRFixtureJson $appPath $appDoc;Sync-RRFixtureLeg $fixture Thai
    Assert-RRFailure 'unbound disconnect lifecycle hash' {Invoke-RRFixture $fixture|Out-Null} 'state hash is not App-bound'

    $fixture=New-RRFixture;$fixtures+=$fixture;$appPath=Join-Path $fixture.English 'app-runtime.json';$corePath=Join-Path $fixture.English 'core-runtime.json';$appDoc=(Read-V02RuntimeReviewStrictJsonFile $appPath 'cross-leg Agent').Value;$coreDoc=(Read-V02RuntimeReviewStrictJsonFile $corePath 'cross-leg Agent').Value;foreach($transition in $coreDoc.Transitions){$transition.AgentIdentity.PaneId='pane-2'};$appDoc.EventA.Changes[0].PaneId='pane-2';$appDoc.EventB.Changes[0].PaneId='pane-2';Write-RRFixtureJson $appPath $appDoc;Write-RRFixtureJson $corePath $coreDoc;Sync-RRFixtureLeg $fixture English
    Assert-RRFailure 'cross-leg Agent identity drift' {Invoke-RRFixture $fixture|Out-Null} 'same intended process/session/Agent identity'

    $fixture=New-RRFixture;$fixtures+=$fixture;$thaiApp=(Read-V02RuntimeReviewStrictJsonFile (Join-Path $fixture.Thai 'app-runtime.json') 'Thai chronology').Value;$thaiCore=(Read-V02RuntimeReviewStrictJsonFile (Join-Path $fixture.Thai 'core-runtime.json') 'Thai chronology').Value;$appPath=Join-Path $fixture.English 'app-runtime.json';$corePath=Join-Path $fixture.English 'core-runtime.json';$appDoc=(Read-V02RuntimeReviewStrictJsonFile $appPath 'cross chronology').Value;$coreDoc=(Read-V02RuntimeReviewStrictJsonFile $corePath 'cross chronology').Value;for($i=0;$i-lt6;$i++){$coreDoc.Transitions[$i].ObservedUtc=$thaiCore.Transitions[$i].ObservedUtc};$appDoc.EventA.PhaseEnteredUtc=$thaiApp.EventA.PhaseEnteredUtc;$appDoc.EventA.ObservedUtc=$thaiApp.EventA.ObservedUtc;$appDoc.EventB.PhaseEnteredUtc=$thaiApp.EventB.PhaseEnteredUtc;$appDoc.EventB.ObservedUtc=$thaiApp.EventB.ObservedUtc;Write-RRFixtureJson $appPath $appDoc;Write-RRFixtureJson $corePath $coreDoc;Sync-RRFixtureLeg $fixture English
