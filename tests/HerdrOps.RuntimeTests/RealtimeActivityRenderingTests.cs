...
[TestClass]
[DoNotParallelize]
public sealed class RealtimeActivityRenderingTests
{
    [TestMethod]
    public void ActualWpfRealtimeActivityRendersLocalizedSynchronizedEvidence()
    {
    ...
    private static void RenderEvidence()
    {
        ...
        try
        {
            ...
            var thaiShell = CreateActivityShell();
            var thaiDefault = Path.Combine(evidenceDirectory, "realtime-activity-th-1672x941.png");
            RenderPng(thaiShell, 1672, 941, thaiDefault);
            AssertRegions(thaiShell);
            AssertVisibleTextContains(thaiShell, language["RealtimeEventsToday"]);
            AssertVisibleTextContains(thaiShell, language["RealtimeEventDetails"]);
            AssertVisibleTextContains(thaiShell, language["RealtimeSyntheticBoundary"]);
            AssertVisibleTextContains(thaiShell, language["RealtimeAll"]);
            AssertVisibleTextDoesNotContain(thaiShell, nameof(ActivityFilterOption));

            var page = Assert.IsInstanceOfType<RealtimeActivityView>(thaiShell.FindName("RealtimeActivityPage"));
            var state = Assert.IsInstanceOfType<RealtimeActivityState>(page.DataContext);
            ...
            AssertVisibleTextContains(thaiShell, "AuthService.cs");
            var thaiSelected = Path.Combine(evidenceDirectory, "realtime-activity-th-scope-selected-1672x941.png");
            RenderPng(thaiShell, 1672, 941, thaiSelected);
            Assert.AreNotEqual(PixelHash(thaiDefault), PixelHash(thaiSelected));

            var compactShell = CreateActivityShell();
            RenderPng(
                compactShell,
                1366,
                768,
                Path.Combine(evidenceDirectory, "realtime-activity-th-1366x768.png"));
            AssertRegions(compactShell);

            language.SetLanguage(UiLanguage.English);
            var englishShell = CreateActivityShell();
            RenderPng(
                englishShell,
                1672,
                941,
                Path.Combine(evidenceDirectory, "realtime-activity-en-1672x941.png"));
            AssertRegions(englishShell);
            AssertVisibleTextContains(englishShell, "Events Today");
            AssertVisibleTextContains(englishShell, "Event Details");
    ...
    private static ShellView CreateActivityShell()
    {
        var shell = ShellView.CreateSyntheticPreview();
        shell.Navigation.SelectedIndex = 2;
        return shell;
    }

    private static void AssertRegions(ShellView shell)
    {
        var page = Assert.IsInstanceOfType<RealtimeActivityView>(shell.FindName("RealtimeActivityPage"));
    ...
    private static void AssertVisibleTextDoesNotContain(DependencyObject root, string unexpected) =>
        Assert.IsFalse(
            ...
            $"Visible text contains an internal presentation value: {unexpected}");
