...
## D-017 — Runtime idle evidence begins only after observable quiescence
...
- Status: The prior remediation passed the full 443-test Release suite, Windows PowerShell 5.1 parse/known-vector checks and two role-distinct static reviews with no remaining P0-P2 findings. The clean `20260816T151614Z` attempt then exposed a no-credit snapshot-before-event race: sequence 2 reconciled Idle to Working with Event count 0, followed 118 ms later by the genuine status Event at sequence 3 with Event count 1. The bounded successor admission, accepted-frame provenance, focused PowerShell 5.1 direct/race/tamper vectors and full 454-test Release suite now pass. Failed runs `20260816T104420Z` and `20260816T151614Z` are not upgraded; a fresh clean-checkout actual-Herdr run and independent runtime acceptance remain required, so no Runtime or Release credit is claimed.
