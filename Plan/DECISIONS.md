...
## D-018 — Evaluation UI is immutable, language-separated, and fail-closed

- Decision: Issue #31 presents the approved `09-evaluation.png` hierarchy from one immutable presentation snapshot: five summary cards, score distribution, seven-day trend, six weighted dimensions, selected Task/Agent source comparison, and top/low performers. Missing or invalid scores remain visible and never become zero; deterministic technical-key tie-breakers keep every view reproducible.
- Presentation contract: `docs/protocol/v0.6-evaluation-ui-contract.md` binds every displayed value, contribution, ranking, and provenance reference to that snapshot and the retained scoring formula/input hashes. Thai and English are separate modes with exactly one selected language; product names, Agent names, technical identities, paths, IDs, hashes, and protocol values remain literal. Charts expose textual equivalents, labels beyond color, and distinct automation names.
- Reconciliation contract: Every card, chart, dimension row, comparison value, and ranking derives from the same retained scoring result and input snapshot. Missing, malformed, invalid, and unavailable values remain explicit and fail closed; equal scores expose tied status while stable technical identities determine order.
...
- Evidence boundary: Static, Synthetic, and Contract evidence remain distinct. Runtime evidence requires observation against an actual installed Herdr instance; Independent Review and Release are separate gates. Actual Herdr Runtime is not proved here.
