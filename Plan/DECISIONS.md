...
- Event contract: Event A and Event B write their phase record before taking the phase baseline, then each require exactly one local ingest-sequence increment and one Event-count increment in the same connection epoch with unchanged bootstrap/disconnect counters and no more than one authoritative reconciliation. Each observation identifies at least one same-terminal/same-pane Agent whose status and state-change sequence advanced. The monitor explicitly labels `AcceptedEventKind=pane.agent_status_changed` only on the connected transition produced by a successfully reconciled filtered frame; unknown-pane, snapshot-failure, disconnect and pane-set-change transitions remain unlabelled even when consumed-frame counters advance. The independent gate binds that labelled Core transition, sequence and state hash to the App observation. Focus, workspace, tab, pane, polling or topology changes cannot substitute for Agent-status evidence.
...
- Status: The current remediation passes the full 443-test Release suite, Windows PowerShell 5.1 parse/known-vector checks and two role-distinct static reviews with no remaining P0-P2 findings. The failed `20260816T104420Z` run is not upgraded; a fresh clean-checkout actual-Herdr run and independent runtime acceptance remain required, so no Runtime or Release credit is claimed.
