...
- Decision: After the replacement-process Event B reaches the App-owned Widget, the composite v0.2 gate waits for five continuous seconds in which Core remains live and both state sequence and genuine Agent-status Event count remain unchanged. Any change resets the interval and is retained in append-only reset provenance; counters are never filtered, decremented or reclassified.
- Phase contract: The progress record carries contiguous ordinals for initial live state, captures, Event A, Dashboard close, disconnect, reconnect, Event B, quiescence, timed resource measurement and completion. The independent script rejects missing, repeated or reordered phases.
- UI lifetime contract: The shared Shell retains only the selected canonical page. When the Dashboard closes while a Widget remains visible, the product releases the Dashboard visual tree and performs a reported Dashboard-to-Widget working-set compaction before the idle baseline. Closed Widget windows detach handlers and release state references.
- Core memory contract: Runtime server identity uses a bounded streaming SHA-256 read from one final-path-bound handle instead of an executable-sized managed buffer. Transient protocol/schema admission buffers are compacted once after the admitted monitor is constructed; reconnect identity verification does not allocate another full executable image.
- Measurement contract: The following 20-second sample still requires an unchanged sequence and Event count, continuous live health, stable exact App/Core process identities, average CPU no greater than 1%, and combined maximum working set no greater than 180 MB. Compaction details, private memory and managed heap remain visible and do not waive the target.
...
- Status: Implementation in progress for the v0.2 runtime remediation; no Runtime or Release credit is claimed.
