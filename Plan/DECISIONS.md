...
- Freshness contract: `Live` describes the validated Core-to-App pipe only. Protocol v1 does not carry Herdr runtime health, so a connected App labels values as the latest state accepted by Core and explicitly keeps Herdr runtime freshness unknown. Connecting, reconnecting, disconnected and stopped Core states immediately render Agent status as offline while retaining last-known identity/topology for diagnosis. Last-known values are never presented as current success.
