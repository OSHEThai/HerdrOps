...
- Input contract: Every source score is an integer from 0 through 100 and carries its own provenance identity and SHA-256. A complete result requires all three sources for all six dimensions. Missing input produces an explicit Incomplete result, invalid range/provenance or duplicate dimensions produce an explicit Invalid result, and both leave the total score null rather than converting absence into a pass.
...
- Evidence boundary: Golden fixtures, deterministic tests, missing/invalid tests, and recalculation tests are Static and Unit evidence for Issue #30 only. They do not prove reviewer authority, production ingestion, Evaluation UI, actual Herdr Runtime, or v0.6 release readiness.
