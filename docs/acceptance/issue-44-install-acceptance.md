...
# Synthetic lifecycle against the exact committed v1.0 fixture set (initial 0.7.0, upgrade 1.0.0) in a generated temp child.
...
DryRun mode materializes deterministic copies of the committed fixture artifacts only below an owned temporary root so artifact/manifest/archive/hash preflight can execute. It emits the same report shape, marks every install/data lifecycle action `SKIPPED`, and removes that temporary root during cleanup. It does not create the simulated install or retained-data targets.

Both non-live modes use the exact committed v1.0 fixture set under `tests/fixtures/v1.0/packaging`: the initial fixture is the explicitly named lower accepted `0.7.0` package and the upgrade fixture is exactly `1.0.0`. Fixture identity comes from the acceptance-owned profile [`issue-44-package-profile.json`](../../tools/acceptance/issue-44-package-profile.json) (`packageVersion 0.7.0`, `syntheticUpgradeVersion 1.0.0`). The profile keeps the `issue` value `38` because fixture generation reuses the Issue #38 packaging-profile family and its `Assert-PackageProfile` fence; the acceptance target remains Issue #44 v1.0.0. Both non-live modes now assert the `v1-target-version` preflight check `PASS` against the exact `1.0.0` upgrade fixture; a drifted fixture version fails closed instead of being marked `NOT_APPLICABLE`. Synthetic artifact and manifest records still carry `packagingIssue 38` and `evidenceClass Static` because they are #38-profile-format fixture bytes, not v1 release artifacts.

## Live-mode boundary

Live mode is deliberately hard to invoke. It is not run by this preparation slice. The complete fail-closed operator checklist is the [`issue-44-live-runbook.md`](issue-44-live-runbook.md); this section summarizes the boundary. A later operator must provide all of the following:
