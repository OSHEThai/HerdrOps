...
The path guards reject repository roots, machine installation roots, real user-data roots, Windows roots, reparse points, and non-temp lifecycle destinations. The scripts contain no registry mutation or process-launch path. A package output is never treated as an installed product.
