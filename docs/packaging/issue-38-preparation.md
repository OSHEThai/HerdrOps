...
MSBuild property evaluation remains fail-closed and bounded. Its process timeout is 120 seconds so a cold SDK/MSBuild start on a contended clean CI host is not confused with an unresponsive evaluator; timeout terminates the owned evaluator, confirms its exit within a second bounded wait, and fails the gate. The packaging test runs an owned hanging-child probe and verifies this termination path in both supported PowerShell hosts. This does not permit generated `obj` imports or weaken import-ancestry verification: the packaging gate still runs before repository build/restore mutation and every repository-local import must match the static verified ancestry.
